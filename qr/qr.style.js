...
.qr .logoclinicauniversidaddelasabana250x90 {
    position: absolute;
      box-sizing: content-box;
      -ms-transform: rotate(0.00deg);
    -webkit-transform: rotate(0.00deg);
...
.qr .qr332c166d {
    ...
    -webkit-transform-origin: center;
    transform-origin: center;
      background-image: url("assets/qr332c166d.png");
    background-position: center;
    background-repeat: no-repeat;
    background-size: cover;
      opacity: 1;
...
;
`;
cssText = cssText.replace(/background-image: url\(\"assets/g, `background-image: url("${import.meta.url.replace('qr.style.js', 'assets')}`);
cssText = cssText.replace(/src: url\(assets/g, `src: url(${import.meta.url.replace('qr.style.js', 'assets')}`);
export const QrStyle = html`
${cssText}
`;
